...
- `building/BuildingTemplates` — places the Protoss buildings from the hand-authored `.nbt` structure templates in `src/main/resources/data/asteriskcraft/structure/` (the source of truth for their shape: redesign them in-game with a structure block and re-export, don't write blocks out in Java). Templates are anchored on the center column of their bottom layer, and the core block's position is looked up in the template via `StructureTemplate.filterBlocks` rather than hardcoded as an offset. Server-side only — see the structure-template section of docs/neoforge-api-notes.md.
- `building/BuildingLayouts` — the Zerg Hive's code-defined layout (a `Map<BlockPos, BlockState>`), plus the site preparation both paths share: `prepareSite(...)` clears head-room over a footprint (sized from the template for the Protoss buildings) and optionally fills support beneath it. The Hive fills with mycelium; the Protoss buildings pass a null support so nothing is stamped under their own stonework. Kept pure and parameterized (`planSitePrep`) so the geometry stays unit-testable without a live `ServerLevel`.
