 ...
 * Mutalisk}: a Gateway-trained fighter craft that cruises {@link #HOVER_HEIGHT} blocks above the
 * terrain, sharing the Mutalisk's exact flight envelope (same altitude, same {@link #ATTACK_RADIUS},
 * same {@link #ATTACK_COOLDOWN}) but out-lasting and out-gunning it — 75 HP behind 50 shields, and a
 * blaster shot worth more than twice a glave. Like every flyer, melee ground units simply cannot
 * reach it; a Photon Cannon still out-ranges it, which keeps static defence the hard counter to air
 * on both sides.
 *
...
public class ScoutEntity extends Monster implements Shielded, RangedAttackMob {
    ...
    /** Reach of the photon blasters, in blocks. Matches the Mutalisk it exists to hunt. */
    public static final float ATTACK_RADIUS = 9.0f;
    /** One volley every 1.5s, the same cadence as a Mutalisk's glave. */
    public static final int ATTACK_COOLDOWN = 30;
    /** Blocks above the terrain the Scout cruises at. */
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
                ...
                .add(Attributes.ARMOR, 0.5) // Protoss plating, as on the Dragoon
    ...
    protected PathNavigation createNavigation(Level level) {
        ...
        // The default path length is far shorter than this unit's 32-block follow range, so a Scout
        // would give up on distant targets mid-approach. (Vanilla's Bee raises it for the same reason.)
