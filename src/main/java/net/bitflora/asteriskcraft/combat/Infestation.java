 ...
 *
 * <p>Which army raises the dead is a fact about its <em>race</em> ({@link Race#infests}), not about
 * which side it is on, so this takes the race and never names one.
 *
...
public final class Infestation {
     ...
     *
     * @param killer the race of whatever landed the killing blow; null for an unowned killer
     * @param victim what died
     * @param roll   a uniform sample in [0, 1)
     */
    public static boolean infests(@Nullable Race killer, Class<? extends Entity> victim, float roll) {
        return killer != null && killer.infests()
